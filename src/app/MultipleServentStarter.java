...
public class MultipleServentStarter {
	...
	private static void startServentTest(String testName) {
		...
		ProcessBuilder bsBuilder = new ProcessBuilder("java", "-cp", "out\\production\\Peer Market", "app.BootstrapServer", String.valueOf(AppConfig.BOOTSTRAP_PORT));
		...
		try {
			...
			AppConfig.timestampedStandardPrint("Bootstrap  launch failed");
			e1.printStackTrace();
		}
		...
		for(int i = 0; i < serventCount; i++) {
			try {
				ProcessBuilder builder = new ProcessBuilder("java", "-cp", "out\\production\\Peer Market", "app.ServentMain",
